...
- [x] **16.1. API для отправки лидов в Telegram** ✅
  - `app/api/send-lead/route.ts`
  - Интеграция с Telegram Bot
  - Время: 3-4 часа
  - 13.11.2025 — выполнено: создан API route, все формы настроены на отправку в Telegram, добавлена защита от спама (rate limiting, honeypot, проверка времени)
...
### ✅ День 11-12: Контент для SEO (после запуска, P2) - **ЗАВЕРШЕНО**
**Время:** 10-14 часов  
...
- ✅ **Задача 0: Создание ответов на вопросы (2-3ч)** — **ВЫПОЛНЕНО**
  ...
  - Добавлено оглавление на странице FAQ
...
- ✅ **8.3.1: Статьи для `/construction` (8-12ч)** — **ВЫПОЛНЕНО**
  ...
  - Создан файл `CONSTRUCTION_ARTICLES_TZ.md` с техническим заданием
- ✅ **11.1a: Страницы производителей (3-4ч)** — **ВЫПОЛНЕНО**
  ...
  - Schema.org Brand + Product разметка (перенесена из клиентского компонента в серверный)
  - Оптимизированные мета-теги под брендовые запросы
  - Добавлены ссылки на страницы производителей на региональных главных страницах
- ✅ **11.1c: Повторная проверка Schema.org после контента (1ч)** — **ВЫПОЛНЕНО**
  - Проверены все новые страницы (статьи + страницы производителей)
  - Перенесена Schema.org разметка из клиентского компонента в серверный для лучшей SEO
  - Обновлён `SEO_research/schema_validation.md` с результатами проверки
- ✅ **1.1a: Финальная ревизия мета-тегов (2-3ч)** — **ВЫПОЛНЕНО**
  ...
  - Создан подробный отчет `SEO_research/meta_tags_audit.md`

**Результат:**
- ✅ Контент оптимизирован под реальные поисковые запросы
- ✅ Полноценный раздел статей о строительстве (18 статей), отвечающий на запросы пользователей
- ✅ Страницы производителей созданы для всех регионов
- ✅ Schema.org разметка проверена и оптимизирована
- ✅ Все мета-теги проверены и оптимизированы

**13.11.2025 (дополнительно) — Защита от спама и улучшения UX**
- ✅ **Защита от спама во все формы (2-3ч)** — **ВЫПОЛНЕНО**
  - Реализован rate limiting: максимум 5 запросов за 15 минут с одного IP
  - Добавлены honeypot поля во все формы (скрытые от пользователей)
  - Добавлена проверка времени заполнения формы (минимум 3 секунды)
  - Улучшена валидация данных (имя, телефон)
  - Добавлена обработка ошибок 429 (слишком много запросов)
...
- ✅ **Добавлена Яндекс.Метрика (1ч)** — **ВЫПОЛНЕНО**
...
- ✅ **Улучшена кнопка "Подобрать" в хедере (1ч)** — **ВЫПОЛНЕНО**
  - Кнопка "Подобрать" в desktop и mobile меню открывает модалку обратной связи
  - Модалка адаптирована для использования без manufacturerName (консультация)
  - Автоматически передаётся выбранный регион
  - Source определяется автоматически: 'product' или 'consultation'

---

### ⏸️ День 13-14: Блоки для доверия (P2) — **ОТЛОЖЕНО**
**Время:** 8-10 часов  
**Статус:** Отложено на более поздний срок (не критично для старта продвижения)

**Решение:**
- Блоки доверия не критичны для старта продвижения сайта
- Сайт полностью готов к продвижению без них
...
**Задачи (отложены):**
- 12.1: Страница "О компании" (2-3ч) — отложено
- 12.2: Страница "Сертификаты качества" (2ч) — отложено
- 12.3: Блок "Гарантии" (1-2ч) — отложено
- 12.4: Блок "Партнеры и поставщики" (2-3ч) — отложено
- 12.5: Интеграция блоков доверия на страницы (2ч) — отложено
- 🆕 **11.1b: Региональные блоки доверия (2ч)** — отложено
  - План создан в `REGIONAL_TRUST_BLOCKS_PLAN.md`
  - Будет реализовано позже, когда появятся отзывы и статистика

**Планы на будущее:**
- Добавить полноценные блоки доверия после сбора первых отзывов
- Создать страницу "О компании" с историей и опытом
- Добавить страницу "Сертификаты качества"
- Добавить статистику (количество клиентов, лет на рынке)
...
1. **Домен и хостинг**
- [x] Настроен домен ✅
- [x] SSL сертификат ✅ (13.11.2025 — установлен через certbot, Let's Encrypt)
- ✅ Деплой на продакшн

2. **Яндекс.Вебмастер**
   - [ ] Добавлен сайт в Яндекс.Вебмастер (инструкция: `YANDEX_WEBMASTER_SETUP.md`)
   - [ ] Загружен sitemap.xml (готов: https://gazobeton-online.ru/sitemap.xml, 365 URL)
   - [ ] Проверка индексации
